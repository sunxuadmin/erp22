  ...
  <div class="p-2">
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="500px" append-to-body>
      <el-form ref="configFormRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item v-if="form.configKey === GROUP_NAME_FORM_KEYS_CONFIG_KEY">
          <el-collapse>
            <el-collapse-item title="查看组别字段配置说明（点击展开）" name="groupNameFormKeys">
              <div class="text-xs text-gray-500 leading-6">
                该参数用于配置“组别”字段读取优先级。系统会按下面顺序读取表单 JSON 中的字段，取到第一个非空值：
                <span class="font-medium">groupName</span> → <span class="font-medium">group_name</span> → <span class="font-medium">group</span> → <span
                  class="font-medium">groupCode</span> → <span class="font-medium">group_code</span>，未找到再回退 groupCode。
              </div>
              <div class="text-xs text-gray-400 mt-2 leading-6">
                建议配置示例：
                <pre class="mt-1 mb-0 text-xs bg-gray-50 border rounded p-2 whitespace-pre-wrap">["groupName","group_name","group","groupCode","group_code"]</pre>
              </div>
            </el-collapse-item>
          </el-collapse>
        </el-form-item>
...
const GROUP_NAME_FORM_KEYS_CONFIG_KEY = 'crehn.review.project.groupName.formKeys';
