    ...
    <el-row :gutter="20">
      ...
      <el-col :lg="4" :xs="24" style="">
        <el-card shadow="hover">
          <el-segmented v-model="deptScope" :options="deptScopeOptions" class="dept-scope" @change="getDeptTree" />
          <el-input v-model="deptName" placeholder="请输入单位名称" prefix-icon="Search" clearable />
          <el-tree
            ref="deptTreeRef"
            class="mt-2"
            node-key="id"
...
const deptName = ref('');
const deptScope = ref<'art' | 'all'>('art');
...
const deptScopeOptions = [
  { label: '艺术评审部门', value: 'art' },
...
const getDeptTree = async () => {
  const query = deptScope.value === 'art' ? { deptCategory: 'art_' } : undefined;
  const res = await api.deptTreeSelect(query);
