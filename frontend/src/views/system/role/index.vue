  ...
  <div class="p-2">
    ...
    <el-card shadow="hover">
      ...
      <el-table ref="roleTableRef" border v-loading="loading" :data="roleList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="关闭提醒" align="center" width="110">
          <template #default="scope">
            <el-switch
              v-model="scope.row.closeWarningEnabled"
              :disabled="!canChangeCloseWarning(scope.row)"
              @change="handleCloseWarningChange(scope.row)"
            />
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="500px" append-to-body>
      <el-form ref="roleFormRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="菜单权限">
          <el-alert
            v-if="permissionScopeRoleName"
            class="mb-2"
            type="info"
            :closable="false"
            show-icon
            :title="`权限上限：${permissionScopeRoleName}（${permissionScopeRoleKey}）`"
            description="只能勾选该角色已拥有的菜单；上限新增不会自动授权，上限移除会同步收回越界权限。"
          />
          <el-checkbox v-model="menuExpand" @change="handleCheckedTreeExpand(Boolean($event), 'menu')">展开/折叠</el-checkbox>
          <el-checkbox v-model="menuNodeAll" @change="handleCheckedTreeNodeAll(Boolean($event), 'menu')">全选/全不选</el-checkbox>
    ...
    <el-dialog v-model="openDataScope" :title="dialog.title" width="500px" append-to-body>
      <el-form ref="dataScopeRef" :model="form" label-width="80px">
        ...
        <el-form-item v-show="form.dataScope === '2'" label="数据权限">
          ...
          <el-checkbox v-model="deptNodeAll" @change="handleCheckedTreeNodeAll(Boolean($event), 'dept')">全选/全不选</el-checkbox>
...
const isSuperAdmin = computed(() => userStore.roles.includes('superadmin') || userStore.roles.includes('admin'));
...
const permissionScopeRoleName = ref('');
const permissionScopeRoleKey = ref('');
...
const canChangeCloseWarning = (row: RoleVO) => {
  if (!auth.hasPermi('system:role:edit')) return false;
  return String(row.roleId) !== '1' || isSuperAdmin.value;
};

/** 角色关闭/刷新提醒修改 */
const handleCloseWarningChange = async (row: RoleVO) => {
  const enabled = Boolean(row.closeWarningEnabled);
  try {
    await changeRoleCloseWarning(row.roleId, enabled);
    proxy?.$modal.msgSuccess(enabled ? '关闭提醒已开启' : '关闭提醒已关闭');
  } catch {
    row.closeWarningEnabled = !enabled;
  }
};
...
const reset = () => {
  ...
  permissionScopeRoleName.value = '';
  permissionScopeRoleKey.value = '';
...
const getRoleMenuTreeselect = (roleId: string | number) => {
  return roleMenuTreeselect(roleId).then((res): RoleMenuTree => {
    ...
    permissionScopeRoleName.value = res.data.permissionScopeRoleName || '';
    permissionScopeRoleKey.value = res.data.permissionScopeRoleKey || '';
    return res.data;
  });
};
/** 根据角色ID查询部门树结构 */
