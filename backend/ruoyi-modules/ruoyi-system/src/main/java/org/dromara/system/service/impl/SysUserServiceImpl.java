...
public class SysUserServiceImpl implements ISysUserService, UserService {
    ...
    public TableDataInfo<SysUserVo> selectPageUserList(SysUserBo user, PageQuery pageQuery) {
        ...
        fillLoginLockInfo(page.getRecords());
        return TableDataInfo.build(page);
    }

    private void fillLoginLockInfo(List<SysUserVo> users) {
        if (CollUtil.isEmpty(users)) {
            return;
        }
        users.forEach(this::fillLoginLockInfo);
    }

    private void fillLoginLockInfo(SysUserVo user) {
        if (ObjectUtil.isNull(user) || StringUtils.isBlank(user.getUserName())) {
            return;
        }
        String errorKey = CacheConstants.PWD_ERR_CNT_KEY + user.getUserName();
        Integer errorCount = ObjectUtil.defaultIfNull(RedisUtils.<Integer>getCacheObject(errorKey), 0);
        long ttlMillis = RedisUtils.getTimeToLive(errorKey);
        long remainingSeconds = ttlMillis > 0 ? (ttlMillis + 999) / 1000 : 0;
        int lockThreshold = ObjectUtil.defaultIfNull(maxRetryCount, 0);
        user.setLoginErrorCount(errorCount);
        user.setLoginLockThreshold(lockThreshold);
        user.setLoginLocked(lockThreshold > 0 && errorCount >= lockThreshold);
        user.setLoginLockRemainingSeconds(remainingSeconds);
    }
    ...
    public List<SysUserExportVo> selectUserExportList(SysUserBo user) {
        ...
        wrapper.eq("u.del_flag", SystemConstants.NORMAL)
            .and(StringUtils.isNotBlank(user.getUserName()),
                w -> w.like("u.user_name", user.getUserName()).or().like("u.login_alias", user.getUserName()))
    ...
    private Wrapper<SysUser> buildQueryWrapper(SysUserBo user) {
        ...
        wrapper.eq(SysUser::getDelFlag, SystemConstants.NORMAL)
            ...
            .and(StringUtils.isNotBlank(user.getUserName()),
                w -> w.like(SysUser::getUserName, user.getUserName()).or().like(SysUser::getLoginAlias, user.getUserName()))
    ...
    public void unlockLogin(String userName) {
        ...
        if (ObjectUtil.isNull(user)) {
            user = baseMapper.selectVoOne(new LambdaQueryWrapper<SysUser>()
                .eq(SysUser::getLoginAlias, userName)
                .last("limit 1"));
        }
        String lockUserName = userName;
        if (ObjectUtil.isNotNull(user)) {
            checkUserAllowed(user.getUserId());
            checkUserDataScope(user.getUserId());
            lockUserName = user.getUserName();
        }
        RedisUtils.deleteObject(CacheConstants.PWD_ERR_CNT_KEY + lockUserName);
        if (!StringUtils.equals(lockUserName, userName)) {
    ...
    public boolean checkUserNameUnique(SysUserBo user) {
        boolean userNameExist = baseMapper.exists(new LambdaQueryWrapper<SysUser>()
            .and(w -> w.eq(SysUser::getUserName, user.getUserName()).or().eq(SysUser::getLoginAlias, user.getUserName()))
            .ne(ObjectUtil.isNotNull(user.getUserId()), SysUser::getUserId, user.getUserId()));
        if (userNameExist) {
            return false;
        }
        if (StringUtils.isBlank(user.getLoginAlias())) {
            return true;
        }
        if (StringUtils.equalsIgnoreCase(user.getUserName(), user.getLoginAlias())) {
            return false;
        }
        boolean loginAliasExist = baseMapper.exists(new LambdaQueryWrapper<SysUser>()
            .and(w -> w.eq(SysUser::getUserName, user.getLoginAlias()).or().eq(SysUser::getLoginAlias, user.getLoginAlias()))
            .ne(ObjectUtil.isNotNull(user.getUserId()), SysUser::getUserId, user.getUserId()));
        return !loginAliasExist;
    }
