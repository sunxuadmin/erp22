        ...
        return """
                ...
                where srm.role_id in (
                        select sur.role_id from sys_user_role sur
                            left join sys_role user_role on user_role.role_id = sur.role_id
                            where sur.user_id = %d and user_role.status = '0'
                    )
                    and (sr.permission_scope_role_id is null or exists (
                        select 1 from sys_role_menu scope_menu
                        where scope_menu.role_id = sr.permission_scope_role_id
                            and scope_menu.menu_id = srm.menu_id
                    ))
            """.formatted(userId);
    }
