...
public class SysRoleServiceImpl implements ISysRoleService, RoleService {
    ...
    private static final String ART_ADMIN_ROLE_KEY = "crehn_admin";
    private static final String ART_OPS_ROLE_KEY = "crehn_ops";
    private static final String ART_SIGNATURE_ADMIN_ROLE_KEY = "crehn_admin_signature";
    ...
    @Override
    public SysRoleVo selectPermissionScopeRole(Long roleId) {
        SysRole role = baseMapper.selectById(roleId);
        if (role == null) {
            return null;
        }
        Long scopeRoleId = resolvePermissionScopeRoleId(role);
        return scopeRoleId == null ? null : baseMapper.selectVoById(scopeRoleId);
    }

    @Override
    public List<Long> selectPermissionScopeChildRoleIds(Long roleId) {
        return baseMapper.selectObjs(new LambdaQueryWrapper<SysRole>()
            .select(SysRole::getRoleId)
            .eq(SysRole::getPermissionScopeRoleId, roleId));
    }
    ...
    public void checkRoleAllowed(SysRoleBo role) {
        SysRole existingRole = ObjectUtil.isNull(role.getRoleId()) ? null : baseMapper.selectById(role.getRoleId());
        if (ART_SIGNATURE_ADMIN_ROLE_KEY.equals(role.getRoleKey())
            || (existingRole != null && ART_SIGNATURE_ADMIN_ROLE_KEY.equals(existingRole.getRoleKey()))) {
            if (!LoginHelper.isSuperAdmin()) {
                throw new ServiceException("只有超级管理员可以管理评审签字管理员角色");
            }
        }
        ...
        if (ObjectUtil.isNotNull(role.getRoleId())) {
            SysRole sysRole = existingRole == null ? baseMapper.selectById(role.getRoleId()) : existingRole;
    ...
    public int updateRole(SysRoleBo bo) {
        SysRole currentRole = baseMapper.selectById(bo.getRoleId());
        if (currentRole == null) {
            throw new ServiceException("角色不存在或已被删除");
        }
        Long scopeRoleId = resolvePermissionScopeRoleId(currentRole);
        validatePermissionScopeMenus(scopeRoleId, bo.getMenuIds());

        SysRole role = MapstructUtils.convert(bo, SysRole.class);
        role.setPermissionScopeRoleId(scopeRoleId);
        ...
        prunePermissionScopeChildren(role.getRoleId());
        return rows;
    }
    ...
    public int updateRoleStatus(Long roleId, String status) {
        assertSignatureAdminMutable(roleId);
     ...
     *
     * @param roleId 角色ID
     * @param closeWarningEnabled 是否启用关闭/刷新提醒
     * @return 结果
     */
    @Override
    public int updateRoleCloseWarning(Long roleId, Boolean closeWarningEnabled) {
        assertSignatureAdminMutable(roleId);
        return baseMapper.update(null,
            new LambdaUpdateWrapper<SysRole>()
                .set(SysRole::getCloseWarningEnabled, closeWarningEnabled)
                .eq(SysRole::getRoleId, roleId));
    }
    ...
    public int authDataScope(SysRoleBo bo) {
        assertSignatureAdminMutable(bo == null ? null : bo.getRoleId());
        SysRole role = MapstructUtils.convert(bo, SysRole.class);
        // 修改角色信息
        baseMapper.updateById(role);
        // 删除角色与部门关联
    ...
    public int deleteRoleById(Long roleId) {
        assertSignatureAdminMutable(roleId);
        SysRole targetRole = baseMapper.selectById(roleId);
        if (targetRole != null) {
            checkRoleAllowed(BeanUtil.toBean(targetRole, SysRoleBo.class));
        }
        assertNoPermissionScopeChildren(roleId, Collections.emptySet());
        // 删除角色与菜单关联
    ...
    public int deleteRoleByIds(List<Long> roleIds) {
        ...
        Set<Long> deletingRoleIds = new HashSet<>(roleIds);
        for (Long roleId : roleIds) {
            assertNoPermissionScopeChildren(roleId, deletingRoleIds);
        }
    ...
    /**
     * 兼容迁移尚未绑定前的既有 crehn_ops；迁移完成后以数据库绑定为准。
     */
    private Long resolvePermissionScopeRoleId(SysRole role) {
        if (role.getPermissionScopeRoleId() != null) {
            return role.getPermissionScopeRoleId();
        }
        if (!ART_OPS_ROLE_KEY.equals(role.getRoleKey())) {
            return null;
        }
        SysRole scopeRole = baseMapper.selectOne(new LambdaQueryWrapper<SysRole>()
            .select(SysRole::getRoleId)
            .eq(SysRole::getRoleKey, ART_ADMIN_ROLE_KEY));
        return scopeRole == null ? null : scopeRole.getRoleId();
    }

    private void assertSignatureAdminMutable(Long roleId) {
        if (roleId == null || LoginHelper.isSuperAdmin()) {
            return;
        }
        SysRole role = baseMapper.selectById(roleId);
        if (role != null && ART_SIGNATURE_ADMIN_ROLE_KEY.equals(role.getRoleKey())) {
            throw new ServiceException("只有超级管理员可以管理评审签字管理员角色");
        }
    }

    private void validatePermissionScopeMenus(Long scopeRoleId, Long[] requestedMenuIds) {
        if (scopeRoleId == null) {
            return;
        }
        SysRole scopeRole = baseMapper.selectById(scopeRoleId);
        if (scopeRole == null) {
            throw new ServiceException("菜单权限上限角色不存在，请先修复角色配置");
        }
        Set<Long> allowedMenuIds = new HashSet<>(roleMenuMapper.selectMenuIdsByRoleId(scopeRoleId));
        Set<Long> requested = new HashSet<>(Arrays.asList(
            ObjectUtil.defaultIfNull(requestedMenuIds, new Long[0])));
        requested.remove(null);
        requested.removeAll(allowedMenuIds);
        if (CollUtil.isNotEmpty(requested)) {
            String invalidIds = requested.stream().sorted().limit(10)
                .map(String::valueOf).collect(java.util.stream.Collectors.joining(","));
            throw new ServiceException("所选菜单超出权限上限角色“" + scopeRole.getRoleName()
                + "”的授权范围，越界菜单ID：" + invalidIds);
        }
    }

    /**
     * 上限角色缩权时同步删除子角色越界关联；上限扩权时不自动新增。
     */
    private void prunePermissionScopeChildren(Long scopeRoleId) {
        List<Long> childRoleIds = selectPermissionScopeChildRoleIds(scopeRoleId);
        if (CollUtil.isEmpty(childRoleIds)) {
            return;
        }
        List<Long> allowedMenuIds = roleMenuMapper.selectMenuIdsByRoleId(scopeRoleId);
        for (Long childRoleId : childRoleIds) {
            LambdaQueryWrapper<SysRoleMenu> wrapper = new LambdaQueryWrapper<SysRoleMenu>()
                .eq(SysRoleMenu::getRoleId, childRoleId);
            if (CollUtil.isNotEmpty(allowedMenuIds)) {
                wrapper.notIn(SysRoleMenu::getMenuId, allowedMenuIds);
            }
            roleMenuMapper.delete(wrapper);
        }
    }

    private void assertNoPermissionScopeChildren(Long roleId, Set<Long> deletingRoleIds) {
        List<SysRole> childRoles = baseMapper.selectList(new LambdaQueryWrapper<SysRole>()
            .select(SysRole::getRoleId, SysRole::getRoleName)
            .eq(SysRole::getPermissionScopeRoleId, roleId)
            .notIn(CollUtil.isNotEmpty(deletingRoleIds), SysRole::getRoleId, deletingRoleIds));
        if (CollUtil.isNotEmpty(childRoles)) {
            throw new ServiceException("角色仍是“" + childRoles.get(0).getRoleName()
                + "”的菜单权限上限，请先解除或一并删除子角色");
        }
    }
    ...
    public int deleteAuthUser(SysUserRole userRole) {
        assertSignatureAdminMutable(userRole == null ? null : userRole.getRoleId());
