...
public class SysRoleController extends BaseController {
    ...
    public R<Void> edit(@Validated @RequestBody SysRoleBo role) {
        ...
        List<Long> permissionScopeChildRoleIds = roleService.selectPermissionScopeChildRoleIds(role.getRoleId());
        if (roleService.updateRole(role) > 0) {
            roleService.cleanOnlineUserByRole(role.getRoleId());
            permissionScopeChildRoleIds.forEach(roleService::cleanOnlineUserByRole);
            return R.ok();
        }
    ...
    @Log(title = "角色管理", businessType = BusinessType.UPDATE)
    @RepeatSubmit()
    @PutMapping("/changeCloseWarning")
    public R<Void> changeCloseWarning(@RequestBody SysRoleBo role) {
        if (role.getRoleId() == null || role.getCloseWarningEnabled() == null) {
            return R.fail("角色和关闭提醒设置不能为空");
        }
        if (LoginHelper.isSuperAdmin(role.getRoleId()) && !LoginHelper.isSuperAdmin()) {
            return R.fail("只有超级管理员可以修改超级管理员角色的关闭提醒设置");
        }
        roleService.checkRoleDataScope(role.getRoleId());
        return toAjax(roleService.updateRoleCloseWarning(role.getRoleId(), role.getCloseWarningEnabled()));
    }
