...
-- CREHN does not seed fixed-password users. The database image creates the
-- first administrator from required deployment-time environment values.
...
create table sys_role (
    ...
    close_warning_enabled tinyint(1)     default 1                  comment '是否启用浏览器关闭或刷新提醒',
    permission_scope_role_id bigint(20)  default null               comment '菜单权限上限角色ID',
    ...
    remark               varchar(500)    default null               comment '备注',
    primary key (role_id),
    key idx_sys_role_permission_scope (permission_scope_role_id)
) engine=innodb comment = '角色信息表';

-- ----------------------------
-- 初始化-角色信息表数据
-- ----------------------------
insert into sys_role values(1, '000000', '超级管理员',  'superadmin',  1, 1, 1, 1, 1, null, '0', '0', 103, 1, sysdate(), null, null, '超级管理员');
insert into sys_role values(3, '000000', '本部门及以下', 'test1', 3, 4, 1, 1, 1, null, '0', '0', 103, 1, sysdate(), null, null, '');
insert into sys_role values(4, '000000', '仅本人',      'test2', 4, 5, 1, 1, 1, null, '0', '0', 103, 1, sysdate(), null, null, '');
...
-- 初始化-用户和角色关联表数据
-- ----------------------------
-- Initial administrator-role binding is created by bootstrap-admin.sh.
...
-- 初始化-用户与岗位关联表数据
-- ----------------------------
-- No fixed user/post binding is seeded.
...
insert into sys_config values(2, '000000', '用户管理-账号初始密码',        'sys.user.initPassword',         '',              'Y', 103, null, sysdate(), null, null, 'CREHN要求创建账号时显式设置密码' );
