...
public class GlobalExceptionHandler {
    ...
    private static final String OSS_EXCEPTION_CLASS_NAME = "org.dromara.common.oss.exception.OssException";
    ...
    public R<Void> handleMaxUploadSizeExceededException(MaxUploadSizeExceededException e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
        log.error("请求地址'{}',上传文件超过系统限制.", requestURI, e);
        return R.fail("上传文件超过系统允许大小，请压缩后重新上传");
    }
    ...
    public R<Void> handleMultipartException(MultipartException e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
        log.error("请求地址'{}',上传请求解析失败.", requestURI, e);
        return R.fail("上传请求解析失败，请重新选择文件后再上传");
    }

    @ExceptionHandler(RuntimeException.class)
    public R<Void> handleRuntimeException(RuntimeException e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
        if (containsCause(e, OSS_EXCEPTION_CLASS_NAME)) {
            log.error("请求地址'{}',文件存储服务异常.", requestURI, e);
            return R.fail("文件存储服务异常，请检查存储配置或稍后重试");
        }
        log.error("请求地址'{}',发生未知异常.", requestURI, e);
        return R.fail("发生未知异常，请联系管理员");
    }
     ...
     * 系统异常
     */
    private boolean containsCause(Throwable throwable, String className) {
