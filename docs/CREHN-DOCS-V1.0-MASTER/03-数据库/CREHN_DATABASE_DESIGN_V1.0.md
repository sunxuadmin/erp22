...
| 页面显示配置 | 已发布且有 Schema 版本的 `page_display_config` | 浏览器本地配置、页面硬编码和多个模块分别维护同一页面布局/表格/文案 |
...
页面显示配置 JSON 只能保存稳定页面、组件、字段、列、动作和文案键的引用及受控属性，不得保存角色权限授予关系、数据范围或可执行代码。组件所需权限来自版本化组件注册契约，发布时与实时角色权限比对；缺失权限可以作为草稿校验结果记录，但不能作为授权数据写入配置。已发布版本保持不可变，权限撤销只影响运行时可用性和告警，不回写历史快照。
